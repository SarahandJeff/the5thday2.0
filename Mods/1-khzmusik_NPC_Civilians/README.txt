...
However, _make sure the version of `0-SCore` matches the exact vanilla version._
For example, if you're playing on game version 2.4,
use a `2.4.x.y` version of `0-SCore` (where `x` and `y` are any numbers).
TFP change their C# code between minor versions,
and SCore must match those code changes.
